...
(function (global) {
  ...
  function renderResearch(idx) {
    ...
    setScreen(h('div', { class: 'panel', style: { maxWidth: '1150px', margin: '10px auto' } },
    ...
    for (const id of g.map.order) {
      ...
      if (tile) {
        ...
        if (hx.mine) {
          ...
          grp.appendChild(s('text', { x, y: y - 1, 'text-anchor': 'middle', 'font-size': 12, class: 'hexhint' }, RES_INFO[hx.mine].icon));
        }
        if (hx.bonus) grp.appendChild(s('text', { x, y: y + (hx.mine ? 15 : 5), 'text-anchor': 'middle', 'font-size': 12, class: 'hexhint' }, bonusIcon(hx.bonus)));
      }
  ...
  /* Gold renders as a styled golden coin (the 🪙 emoji looks like silver). */
  ...
  function renderBoard(p, isActive, collapsible) {
    ...
    const resStrip = h('div', { class: 'res-strip' }, RES_ORDER.map(k => {
      const isGold = k === 'gold';
      const gain = isGold ? (p.renown + p.prod.gold) : p.prod[k];
      const tip = isGold
        ? 'Gold — you gain ' + gain + ' every week (Renown ' + p.renown + ' + Gold production ' + p.prod.gold + ')'
        : RES_INFO[k].name + ' — you produce ' + p.prod[k] + ' every week';
    ...
    return h('div', { class: 'pboard p' + p.idx + (isActive ? ' active' : '') + (collapsed ? ' collapsed' : '') },
      h('div', { class: 'phead' },
        ...
        h('div', { class: 'ptown' }, f ? f.name : ''),
        collapsed ? h('div', { class: 'ptown', style: { opacity: .7 } }, '— board hidden —') : null,
        h('div', { class: 'glory', title: 'Committed Glory: Renown + card ★ + map tiles + claimed Deeds. Awards and your secret objective are added at game end.' },
          h('b', null, '★ ' + liveGlory(p)), h('small', null, 'Glory · Renown ' + p.renown))),
  ...
  function renderActions() {
    ...
    const conv = h('div', { class: 'acts' },
      ...
      actBtn('🔨 Forge', 'smelt Ore → arm your host', '2 🪨 → 3 ⚔️',
    ...
    const transmute = h('div', { class: 'transmute-row' },
      ...
      ['gold', 'wood', 'ore', 'crystal', 'recruits'].map(rk =>
  ...
  const HELP = {
    win: 'The conquest ends in one of two ways: (1) all three shared tracks reach their maximum, or (2) any lord completes their Town Wonder. Either way it ends after the week both lords have passed. Every track step you raise also grants +1 Renown (your Glory and Gold income).',
    wonder: 'Your Town Wonder — an alternate path to victory. Build its 3 stages in order (each is a turn action with its own cost + requirement and an immediate reward; the last grants a permanent synergy). Finishing it scores ' + (global.HK.data.WONDER_VP) + ' Glory and ENDS the game after both lords pass — so it doubles as a way to cut the game short on your terms.',
  ...
  function renderGoals() {
    ...
    const miles = h('div', null, MILESTONES.map(m => {
      ...
      return h('div', { class: 'mile-row' },
        ...
        claimed ? null : h('button', { disabled: !can, onClick: can ? (() => { g.claimMilestone(idx, m.id); UI.sync(); }) : null }, eng.MILESTONE_COST + '🪙'));
    }));
    ...
    const awards = h('div', null, AWARDS.map(a => {
      ...
      return h('div', { class: 'award-row' },
        ...
        funded ? null : h('button', { disabled: !can, onClick: can ? (() => { g.fundAward(idx, a.id); UI.sync(); }) : null }, fundCost + '🪙'));
    }));
    ...
    return h('div', { class: 'panel goals-panel' },
      h('h3', null, '🎯 Goals'),
      h('div', { class: 'subhead' }, 'Win condition — max all three tracks', qIcon(HELP.win)),
      tracks,
  ...
  /* ----------------------------- wonder panel ----------------------- */
  function costStr(cost) { return Object.keys(cost).map(k => cost[k] + (k === 'gold' ? '🪙' : RES_INFO[k].icon)).join(' '); }
  function renderWonder() {
    ...
    const rows = w.stages.map((st, i) => {
      ...
      return h('div', { class: 'wonder-stage' + (done ? ' done' : '') + (isNext ? ' next' : '') },
        ...
        h('div', { class: 'ws-meta' }, 'Cost ' + costStr(st.cost) + (st.req ? '  ·  Needs ' + reqText(st.req) : '')),
    ...
    return h('div', { class: 'panel wonder-panel' },
      ...
      h('div', { class: 'muted', style: { fontSize: '12px', marginBottom: '6px' } }, w.blurb + ' Finishing it ends the game after both pass.'),
      rows,
      p.wonderComplete ? h('div', { style: { color: 'var(--good)', marginTop: '6px', fontWeight: '700' } }, '✓ Completed! +' + WONDER_VP + '★ — the conquest ends this week.') : null);
  }
  ...
  function renderQuests() {
    ...
    const cards = g.questBoard.map(qid => {
      ...
      return h('div', { class: 'quest' + (ok ? ' ready' : '') },
        ...
        h('div', { class: 'q-cost' }, 'Pay ' + costStr(q.cost)),
  ...
  function renderTop() {
    ...
    return h('div', { class: 'topbar' },
      ...
      h('div', { class: 'wincond', title: 'Conquest ends when all three are maxed' }, 'Win: max all 3 →'),
      globals,
  ...
  function openCardModal(id) {
    ...
    const chk = g.canPlay(p, card), plan = g.defaultPlan(p, card);
    function rerender() {
      const cost = g.effectiveCost(p, card);
      const subVal = (plan.wood || 0) * 2 + (plan.crystal || 0) * 3;
      const goldNeed = Math.max(0, cost - subVal);
      const playable = chk.ok && p.res.gold >= goldNeed;
      const steppers = [];
      const mk = (resKey, perVal) => {
        const max = Math.min(p.res[resKey], Math.ceil(cost / perVal));
        steppers.push(h('div', { class: 'pay-row' },
          h('div', { style: { minWidth: '170px' } }, 'Use ' + RES_INFO[resKey].icon + ' ' + RES_INFO[resKey].name + ' (' + perVal + '🪙 each):'),
          h('div', { class: 'stepper' },
            h('button', { onClick: () => { plan[resKey] = Math.max(0, (plan[resKey] || 0) - 1); rerender(); } }, '−'),
            h('b', null, String(plan[resKey] || 0)),
            h('button', { onClick: () => { plan[resKey] = Math.min(max, (plan[resKey] || 0) + 1); rerender(); } }, '+'))));
      };
      if (card.tags && card.tags.includes('building') && p.res.wood > 0) mk('wood', 2);
      if (card.tags && card.tags.includes('magic') && p.res.crystal > 0) mk('crystal', 3);
      const node = h('div', { class: 'modal' },
        h('h2', null, card.name),
        h('div', { class: 'row' }, cardFace(id, { playable: null }),
          h('div', { style: { flex: 1, minWidth: '220px' } },
            h('p', { class: 'muted' }, 'Base cost ' + card.cost + ' Gold' + (cost !== card.cost ? ' → ' + cost + ' after Town discount' : '')),
            steppers,
            h('p', null, 'Pay ', h('b', { style: { color: 'var(--gold)' } }, goldNeed + ' Gold'),
              subVal ? ' + ' + (plan.wood ? plan.wood + '🪵 ' : '') + (plan.crystal ? plan.crystal + '💎' : '') : '', '   (you have ' + p.res.gold + ' Gold)'),
        ...
        h('div', { class: 'row', style: { marginTop: '14px', justifyContent: 'flex-end' } },
          ...
          h('button', { class: 'danger', onClick: () => { g.sellCard(idx, id); hideOverlay(); UI.sync(); } }, 'Sell for ' + eng.SELL_PRICE + ' 🪙'),
          h('button', { class: 'primary', disabled: !playable, onClick: playable ? (() => { g.playCard(idx, id, plan); hideOverlay(); UI.sync(); }) : null }, '▶ Play')));
      showOverlay(node);
    }
    rerender();
  }
  ...
  function showHelp() {
    const node = h('div', { class: 'modal', style: { maxWidth: '640px' } },
      ...
      h('div', { class: 'help-body', html: `
        ...
        <p><b>Resources:</b> 🪙 Gold pays anything · 🪵 Wood discounts cards with the Building tag · 💎 Crystal discounts Magic ·
