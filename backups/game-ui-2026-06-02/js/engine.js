...
(function (global) {
  ...
  const STANDARD = {
    ...
    town:      { name: 'Flag a Mine', cost: 25, blurb: '+1 Gold production, claim a mine' },
  };
  ...
  class Game {
    ...
    _applyOp(p, op, ctx) {
      ...
      if (op.store != null && ctx.card) ctx.card.store = (ctx.card.store || 0) + op.store;
      if (op.gainPerTag) {
    ...
    // Max value that wood/crystal substitutes can cover for this card.
    _subMax(p, card) {
      let v = 0;
      if (card.tags && card.tags.includes('building')) v += p.res.wood * 2;
      if (card.tags && card.tags.includes('magic')) v += p.res.crystal * 3;
      return v;
    }
    canAfford(p, card) {
      const cost = this.effectiveCost(p, card);
      const sub = this._subMax(p, card);
      return p.res.gold >= Math.max(0, cost - sub);
    }
    // Default payment plan: spend gold first, cover remainder with crystal then wood.
    defaultPlan(p, card) {
      const cost = this.effectiveCost(p, card);
      let remain = cost - p.res.gold;
      const plan = { wood: 0, crystal: 0 };
      if (remain <= 0) return plan;
      if (card.tags && card.tags.includes('magic')) {
        plan.crystal = Math.min(p.res.crystal, Math.ceil(remain / 3));
        remain -= plan.crystal * 3;
      }
      if (remain > 0 && card.tags && card.tags.includes('building')) {
        plan.wood = Math.min(p.res.wood, Math.ceil(remain / 2));
        remain -= plan.wood * 2;
      }
      return plan;
    }
    _pay(p, card, plan) {
      plan = plan || this.defaultPlan(p, card);
      const cost = this.effectiveCost(p, card);
      let wood = plan.wood || 0, crystal = plan.crystal || 0;
      if (!(card.tags && card.tags.includes('building'))) wood = 0;
      if (!(card.tags && card.tags.includes('magic'))) crystal = 0;
      wood = Math.min(wood, p.res.wood);
      crystal = Math.min(crystal, p.res.crystal);
      const subVal = wood * 2 + crystal * 3;
      const goldNeed = Math.max(0, cost - subVal);
      if (p.res.gold < goldNeed) return false;
      p.res.gold -= goldNeed; p.res.wood -= wood; p.res.crystal -= crystal;
      return true;
    }
    ...
    _reqOk(p, r) {
      ...
      if (r.prod) for (const k in r.prod) if (p.prod[k] < r.prod[k]) return fail(`Needs ${r.prod[k]} ${k} production`);
      return { ok: true };
    }
    ...
    buildWonderStage(idx) {
      ...
      if (p.wonderStage >= w.stages.length) {
        ...
        this.log(`✦ ${p.name} has completed ${w.name}! The conquest will end this week.`);
      }
      return { ok: true };
    }
    ...
    transmute(idx, toRes) {
      if (!this._ensureTurn(idx)) return { ok: false, reason: 'Not your turn' };
      const p = this.players[idx];
      if (!['gold', 'wood', 'ore', 'crystal', 'recruits'].includes(toRes)) return { ok: false, reason: 'Bad target' };
    ...
    completeQuest(idx, questId) {
      ...
      this._fillQuests();
      this.log(`${p.name} completes the Adventure: ${q.name}.`);
      return { ok: true };
    }
    ...
    playCard(idx, cardId, plan) {
      ...
      if (!chk.ok) return chk;
      this._saveUndo();
      if (!this._pay(p, card, plan)) { this.undoStack.pop(); return { ok: false, reason: 'Payment failed' }; }
    ...
    sellCard(idx, cardId) {
      ...
      this.log(`${p.name} sells ${cardById(cardId).name} for ${SELL_PRICE} Gold.`);
      return { ok: true };
    }
    ...
    _endGeneration() {
      ...
      for (const p of this.players) {
        ...
        for (const k of ['wood', 'ore', 'crystal', 'recruits', 'mercury']) p.res[k] = Math.max(0, p.res[k] + p.prod[k]);
      }
      ...
      this.passed = [false, false];
      if (this.allMaxed() || this.wonderDone || this.gen >= MAX_GENS) { this._finalize(); return; }
      this.gen++;
    ...
    _snapshot() {
      return structuredClone({
        ...
        wonderDone: this.wonderDone, questDeck: this.questDeck, questBoard: this.questBoard, logLen: this.logLines.length,
      });
    }
    _saveUndo() {
    ...
    undo() {
      ...
      this.wonderDone = s.wonderDone; this.questDeck = s.questDeck; this.questBoard = s.questBoard;
  ...
  const API = { Game, STANDARD, AWARD_COSTS, MILESTONE_COST, SELL_PRICE, RESEARCH_BUY };
